...
const filterOrders = async (req, res) => {
    ...
    try {
        const orderRes = await queryDB('CALL ASP_GET_ORDERS(?,?,?,?,?,?,?,?,?,?,?)', [rangeObj[0], rangeObj[1], filterObj.titleWork, filterObj.workerNames, filterObj.statusId, filterObj.serviceId, filterObj.editorialId, filterObj.clientNames, filterObj.clientPhone, sortObj[0] === "id" ? sortObj[1] : null, sortObj[0] === "createdAt" ? sortObj[1] : null]);
        const orders = orderRes[0];
        res.json({ data: orders, total: orders.length > 0 ? orders[0].total : 0 });
    } catch (error) {
        console.log(error)
        res.status((error && error.statusCode) || 500).json({ msg: (error && error.msg) || 'Error de servidor' });
    }
